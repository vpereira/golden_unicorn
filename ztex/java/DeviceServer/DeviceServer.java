...
// start http server
	    ...
	    if ( httpPort > 0 ) {
		info ( "Listening for socket connections at port " + httpPort + " from addresses " + httpPermissions );
...
// run socket server
	    if ( socketPort > 0 ) {
		info ( "Listening for socket connections at port " + socketPort + " from addresses " + socketPermissions );
